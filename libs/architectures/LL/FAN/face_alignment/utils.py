...
def transform(point, center, scale, resolution, invert=False):
    ...
    t[0, 0] = resolution / he
    t[1, 1] = resolution / he
    t[0, 2] = resolution * (-center[0] / he + 0.5)
    t[1, 2] = resolution * (-center[1] / he + 0.5)
...
def resize(image, resolution=256):
    do_scale_w = image.shape[1] > image.shape[0]
    target_w = resolution
    target_h = resolution

    if do_scale_w:
        scale_factor = float(resolution) / float(image.shape[1])
        target_h = int(np.round(image.shape[0] * scale_factor))
    else:
        scale_factor = float(resolution) / float(image.shape[0])
        target_w = int(np.round(image.shape[1] * scale_factor))

    top = int(max(0, np.round((resolution - target_h) / 2)))
    left = int(max(0, np.round((resolution - target_w) / 2)))
    bottom = int(resolution - top - target_h)
    right = int(resolution - left - target_w)
    resized = cv2.resize(image, (target_w, target_h), 0, 0, interpolation=cv2.INTER_LINEAR)
