...
class FaceAlignment:
    ...
    def get_landmarks_with_fd(self, input_image, all_faces=False):
        with torch.no_grad():
            ...
            if len(detected_faces) > 0:
                ...
                for i, d in enumerate(detected_faces):
                    ...
                    if self.landmarks_type == LandmarksType._3D:
                        pts_img = self.get_landmarks_depth(inp, pts, pts_img, scale)
    ...
    def get_landmarks(self, image, center, scale):
        with torch.no_grad():
            landmarks = []
            norm_landmarks = []
            if self.enable_cuda:
                image = image.cuda()
            heatmap = self.face_alignemnt_net(image)[-1].data.cpu()
            if self.flip_input:
                heatmap += flip(self.face_alignemnt_net(flip(image))
                    [-1].data.cpu(), is_label=True)

            pts, pts_img, scores = get_preds_fromhm(heatmap, center, scale)
            pts, pts_img = pts.view(68, 2) * 4, pts_img.view(68, 2)

            if self.landmarks_type == LandmarksType._3D:
                pts_img = self.get_landmarks_depth(image, pts, pts_img, scale)

            landmarks.append(pts_img.numpy())
            norm_landmarks.append(pts.numpy())
            

            return landmarks, norm_landmarks, scores

        
    def get_landmarks_wiht_bbox(self, image, bbox, all_faces=False, all_metrics=False):
        with torch.no_grad():
            if len(bbox) > 0:
                landmarks = []
                cropped_landmarks = []
                center = torch.FloatTensor([bbox[2] - (bbox[2] - bbox[0]) / 2.0, bbox[3] - (bbox[3] - bbox[1]) / 2.0])
                ...
                #plt.show()
                #inp = resize(image,resolution=1024)
                inp = crop(image, center, scale,resolution=256.0)
                #image_plot = plt.imshow(inp, aspect='auto')
                #plt.show()
                inp = torch.from_numpy(inp.transpose(
                        (2, 0, 1))).float().div(255.0).unsqueeze_(0)
                ...
                heatmap = self.face_alignemnt_net(inp)[-1].data.cpu()
                if self.flip_input:
                    heatmap += flip(self.face_alignemnt_net(flip(inp))
                                    [-1].data.cpu(), is_label=True)
                ...
                landmarks.append(pts_img.numpy())
                cropped_landmarks.append(pts.numpy())
            else:
                print("Warning: No faces were detected.")
                return None
            
            if all_metrics:
                return landmarks, cropped_landmarks, heatmap, scores
            else:
                return landmarks
    
    def get_landmarks_depth(self, inp, pts, pts_img, scale):        
    ...
    def heatmap_transforms(self, heatmaps):
        score = get_score_fromhm(heatmaps)
        return score
