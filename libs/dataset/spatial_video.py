    ...
    def __call__(self, clip):
        ...
        for pic in clip:
            if isinstance(pic, np.ndarray):
                ...
                trans.append(img.float().div(self.norm_value))

            elif accimage is not None and isinstance(pic, accimage.Image):
                nppic = np.zeros(
                        [pic.channels, pic.height, pic.width], dtype=np.float32)
                pic.copyto(nppic)
                trans.append(torch.from_numpy(nppic))
            else:
                # handle PIL Image
                if pic.mode == 'I':
...
class BoundingBoxCrop(object):
    #Crops to a given bounding box
    def __init__(self, size, interpolation=Image.BILINEAR):
        self.size = size
        self.interpolation = interpolation

    def __call__(self, clip, object_box):
        
        cropped = []
        for img in clip:
            img = img.crop((object_box[0], object_box[2], object_box[1], object_box[3]))
            cropped.append(img.resize((self.size, self.size), self.interpolation))
...
class Grayscale(object):
    """Convert clip of images to grayscale.
    ...
    def __call__(self, clip):
        ...
        """
        return [F.to_grayscale(img, num_output_channels=self.num_output_channels) for img in clip]
