...
def resize_bbox(bbox, in_size, out_size):
    ...
    #bbox[bbox == 0] = 1 #Stops resizing to 0
    
    #An issue that small boxes end up with 0 width/hieght when resized.
    ydif = bbox[:,2] - bbox[:,0]
    xdif = bbox[:,3] - bbox[:,1]
    yind = np.where(ydif == 0)[0]
    xind = np.where(xdif == 0)[0]
    
    if yind.shape[0] > 0:
        bbox[yind,0] = bbox[yind,0] - 1
    if xind.shape[0] > 0:
        bbox[xind,1] = bbox[xind,1] - 1
